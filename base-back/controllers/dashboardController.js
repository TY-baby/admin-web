...
exports.stats = async (req, res) => {
  try {
    ...
    // db.query 直接返回 rows 数组，不是 [rows, fields]
    ...
    const totalAmount = (amountRows && amountRows[0] && amountRows[0].total) || 0

    res.json({
      code: 0,
      data: {
        totalCustomers: (customers && customers[0] && customers[0].total) || 0,
        totalOpportunities: (opportunities && opportunities[0] && opportunities[0].total) || 0,
        totalOrders: (orders && orders[0] && orders[0].total) || 0,
        totalAmount,
        deals: (deals && deals[0] && deals[0].total) || 0,
        todayOrders: (todayOrders && todayOrders[0] && todayOrders[0].total) || 0,
    ...
    res.status(500).json({ code: 1, message: '获取统计数据失败：' + err.message })
  }
}
